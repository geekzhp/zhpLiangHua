...
def lgtStock():
    ...
    if lastQueryCount['spare']<=5000:
        ...
        startDate='2005-01-01'
        q=query(finance.STK_HK_HOLD_INFO).filter(finance.STK_HK_HOLD_INFO.day>=startDate).limit(2)
        df=finance.run_query(q)
        print(df)
        #df['date']=pd.to_datetime(df['date']) 
        ...
        if 1:
            ...
            q=query(finance.STK_HK_HOLD_INFO).filter(
                finance.STK_HK_HOLD_INFO.day>=df.iloc[-1]['day']).limit(2)
            newDf=finance.run_query(q)
            #newDf['date']=newDf.to_datetime(df['date'])
            print(newDf)
            df=pd.concat([df,newDf],ignore_index=True)
           
            #if newDf.empty or newDf.iloc[-1]['day']==datetime.date(2017,3,16):
            #    break
...
if __name__=="__main__":
    ...
    q=query(finance.STK_ML_QUOTA).filter(finance.STK_ML_QUOTA.day>='2015-01-01')
    df=finance.run_query(q)
    print(df)
    df.to_csv('./data/exchangeTradInfo/陆股通个股2.csv',
              encoding='gbk',
    ...
    """df=pd.read_csv('./data/exchangeTradInfo/陆股通个股.csv',encoding='gbk')
    print(df.iloc[0]['day'])"""
